...
candy_data <- read.csv("aus_employment.csv")
...
candy_data$date <- as.Date(paste(candy_data$date, "01", sep = "-"), format = "%Y-%m-%d")

# Create a time series object
candy_ts <- ts(candy_data$people_employed, frequency = 12, start = c(1978, 1))
...
train <- window(candy_ts, start = c(1978, 1), end = c(1988, 12))
test <- window(candy_ts, start = c(1989, 1), end = c(1991, 12))
...
ts_data <- ts(candy_data$people_employed, frequency = 12)  # Assuming monthly data (frequency = 12)
...
skew <- skewness(candy_data$people_employed)
print(skew)
...
qqnorm(candy_data$people_employed)
qqline(candy_data$people_employed)

# Shapiro-Wilk test --> whether a dataset follows a normal distribution
shapiro.test(candy_data$people_employed)
...
original_data <- candy_data$people_employed
...
candy_data$people_employed <- transformed_data
...
ggplot(data = NULL, aes(x = time(transformed_data), y = candy_ts)) +
  geom_line() +
  labs(x = "Date", y = "X.Passengers") +
  ggtitle("Time Series Plot of X.Passengers")
...
acf(candy_data$people_employed, main = "Autocorrelation Function (ACF)", lag.max = 40)
pacf(candy_data$people_employed, main = "Partial Autocorrelation Function (PACF)", lag.max = 40)

# Statistical Tests using adf --> result < 0.05 means stationary
adf.test(candy_data$people_employed)

# examine relationships between values at different time points
lag.plot(candy_data$people_employed, main = "Lag Plot") # using the original data

#Decompose again 
ts_data2 <- ts(candy_data$people_employed, frequency = 12)  # Assuming monthly data (frequency = 12)
...
acf(candy_data$people_employed, main = "Autocorrelation Function (ACF)")
pacf(candy_data$people_employed, main = "Autocorrelation Function (PACF)")
...
fit <- arima(train, order=c(1,1,0), seasonal = list(order = c(0,1,0), period=12))
fit
summary(fit)
