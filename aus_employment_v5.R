...
forecast_ARIMA <- forecast(fit, h = 36)
...
# Generate forecasts for the next 3 years (36 months)
forecast_AutoArima <- forecast(fit2, h = 36)
...
## Holt Model  ----------------------------------------------------------
# Fit an automatic Holt model to the data
holt_model <- auto.arima(train, seasonal = TRUE)

# Fit a Holt model 
holt_model2 <- holt(train)

# Print the summary of the Holt model
summary(holt_model)
accuracy(holt_model)
checkresiduals(holt_model)

# Specify the total number of periods you want to forecast
forecast_horizon <- 36  # Adjust as needed
...
plot(holt_forecast, main = "Holt Forecast", xlab = "Date", ylab = "People Employed")
...
forecast_ETS <- forecast(fit4, h = 36)
print(forecast_ETS)
plot(forecast_ETS)

# ------------------------- Step 5 : Diagnostic Checking (Randomness) -------------------------
## Check Method residuals
residuals <- checkresiduals(holt_model)
residuals3 <- checkresiduals(fit4)          #ETS 
residuals4 <- checkresiduals(fit, lag = 24) #ARIMA
residuals5 <- checkresiduals(fit2, lag=24)  #auto ARIMA
...
print("Accuracy of Holt Model")
print(accuracy(holt_forecast, test))
...
lines(holt_forecast$mean, col = "brown")  # Holt Forecast model
