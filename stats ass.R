...
# Use train data from 1973 to 1993 for forecasting (20 years)
train = window(candy_data,start=1973,end=c(1993,12))

# Use remaining data from 1994 to 2017 to test accuracy (24 years)
test = window(candy_data, start=1994, end=c(2017,12))

# By percentage
library(forecast)
train <- head(candy_data, round (length(candy_data) * 0.67))
h <- length(candy_data) - length(train)
test <- tail(candy_data,h)
train
test
...
## calculate the first difference and second difference of dataser the plot the first difference
...
##### AR(1)
set.seed(1) #yt, xt is?
xt <- arima.sim(list(order=c(1,0,0), ar=0.9), n=500)
yt <- xt + 20
...
set.seed(1)
yt <- arima.sim(list(order=c(0,0,1), ma=c(0.8)), n=500)
yt <- yt + 2
layout(matrix(c(1, 1, 2, 3), 2, 2, byrow=TRUE))
plot.ts(yt)
acf(yt, main='Autocorrelations')
...
set.seed(1)
yt <- arima.sim(list(order=c(0,0,2), ma=c(0.6,-0.3)), n=500)
yt <- yt + 2
layout(matrix(c(1, 1, 2, 3), 2, 2, byrow=TRUE))
plot.ts(yt)
acf(yt, main='Autocorrelations')
...
predict(object=m, n.ahead=3,prediction.interval=T, level=.90) # error???
...
set.seed(0)
xt <- arima.sim(list(order=c(1,1,1), ar=c(0.8), ma=c(-0.9)), n=500)
yt <- xt + 10
layout(matrix(c(1, 1, 2, 3), 2, 2, byrow=TRUE))
plot.ts(yt)
acf(yt, main='Autocorrelations')
